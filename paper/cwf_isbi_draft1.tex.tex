...
\begin{equation}
CTF(\hat{k};\Delta\hat{z}^2)= \exp{-B\hat{k}^2}\sin[-\pi \Delta\hat{z}\hat{k}^2 + \frac{\pi}{2} \hat{k}^4]
\label{eq:ctf}
\end{equation}
where 
...
%										 =	 %\Pr(||x_{ij}||_{p} < \epsilon|y_i,y_j)\\ %%\nonumber
										...
										& \int_{B_p(0,\epsilon) }^{}\exp \{ -\frac{1}{2}(x_3 - \alpha_{ij})^T(L_i+L_j)^{-1}(x_3 - \alpha_{ij}\}dx_3 
\end{align}
\begin{equation}
