function [ data_cwf ]  = get_cwf_metric(index, CTF_rad_all, basis, sample_points,  mean_coeff, coeff_pos_k,  noise_v)

L=cell(length(unique(index)),1);
nim=length(index);
...
denoised_coeff_pr=cell(length(coeff_pos_k), 1);
...
%            end
 %No regularization for white noise case
	...
	[uu,ss,vv]=svd(C); machineeps=1e-10;
	pr=uu(:,logical(diag(ss)>machineeps));
	%C_pr=pr'*C*pr; % Project onto PC subspace
        for i=1:max(index)
            ...
            % Fix this
  	    term2=h_posk*A{i}*C;
            denoised_coeff_pr{k+1}(:,find(index==i)) = pr'*denoised_coeff{k+1}(:,find(index==i)); 
	    L{i}{k+1}= pr'*C*pr - pr'*term2*pr;			            
        end
    end
end
