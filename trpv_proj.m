...
%is 129x129 pixels.
pathstr='/scratch/tbhamre/cwf_class/';
...
vol=ReadMRC('/scratch/tbhamre/EMD-5778.map');
projections=cryo_project(vol,q,size(vol,1),'single');
data.projections=projections;
data.q=q;
data.shifts=shifts;
%save(fullfile(pathstr,'clean_data_trpv'), '-v7.3', 'data')
...
save(fullfile(pathstr,'clean_data_trpv_129'), '-v7.3', 'data')

downsampleddim=65;
sprintf('Downsampling to %dX%d grid', downsampleddim, downsampleddim)
data.projections=cryo_downsample(data.projections,[downsampleddim downsampleddim],1);
save(fullfile(pathstr,'clean_data_trpv_65'), '-v7.3', 'data')
clear all;
