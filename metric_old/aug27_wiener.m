...
noise_vals=50; % Number of values of noise variance sigma for which the code calculates metric
...
for i=1:d*10
    parfor k=1:d*10
        ...
        %affinity(i,k)= -0.5*log(det(L+M)) -0.5*((alpha-beta)'*inv(L+M)*(alpha-beta));
       
        affinity(i,k)= -0.5*((alpha-beta)'*inv(L+M)*(alpha-beta));
